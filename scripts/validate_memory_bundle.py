...
REQUIRED=["memory/capture_policy.json","memory/frame_index.json","memory/event_timeline.jsonl","memory/ocr_index.jsonl","memory/module_attachment.json","memory/memory_spine.json","memory/seed_spine.json","memory/ai_digest.md","memory/ai_digest.json","receipts/arc_receipts.jsonl","omnibinary/chunk_map.json","arcrar/bundle_manifest.json"]
def main():
    ...
    write_json(session/"reports"/"validation_report.json", {"schema":"arc.streammemory.validation_report.v2","created_at":utc_now(),"ok":not errors,"errors":errors,"frame_files":len(frames)})
    if errors:
